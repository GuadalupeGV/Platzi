...
def generate_random_vector():
        vectors = []
        for _ in range(10):
            ventor = Vector(random.randint(0, 10), random.randint(0, 10))
            vectors.append(ventor)
        return vectors

 #We can use euclidian distance because we are dealing with a cartesian plane
def get_eucladian_distance(vector_a, vector_b):
...
def run():
    ...
    #Generatig graph of the vectors with bokeh
    ...
    #I´ll implemento memoization with a dict of distances
    distances = {}

    #I´ll work with the firs cluster of my clusters lista
    cluster = clusters[-1][::]
        ...
        for vector_tracked in cluster:
            for vector in vectors:
                if f'{vector_tracked}*{vector}' not in distances.keys():
                    distance = get_eucladian_distance(
